...
export async function POST(request: NextRequest) {
  try {
    ...
    // Build the prompt
    const prompt = buildProgramGeneratorPrompt(
      body,
      asanas as any,
      conditions
    );
    ...
    if (!responseContent) {
      return Response.json(
        { error: "Failed to generate program - empty response" },
        { status: 500 }
      );
    }
    ...
    if (!generatedProgram) {
      ...
      return Response.json(
        { error: "Failed to parse generated program" },
        { status: 500 }
      );
    }
    ...
    if (validSequence.length === 0) {
      return Response.json(
        { error: "Generated program contains no valid asanas" },
        { status: 500 }
      );
    }
