...
export async function cancelSubscription(): Promise<ApiResponse<{ success: boolean }>> {
  try {
    const response =
      await apiClient.post<ApiResponse<{ success: boolean }>>('/subscription/cancel');
    return response.data;
  } catch (error) {
    if (error && typeof error === 'object' && 'response' in error) {
      const axiosError = error as {
...
export async function resumeSubscription(): Promise<ApiResponse<{ success: boolean }>> {
  try {
    const response =
      await apiClient.post<ApiResponse<{ success: boolean }>>('/subscription/resume');
    return response.data;
  } catch (error) {
    if (error && typeof error === 'object' && 'response' in error) {
      const axiosError = error as {
